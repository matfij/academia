...
{
    Task<IEnumerable<Quote>> FilterBy(IEnumerable<Quote> quotes, string target);
}

internal class QuoteFilter : IQuoteFilter
{
    private readonly char[] _splitTokens = [' ', '.', ',', ';', '!', '?', '\n', '\r', '\t'];
    public async Task<IEnumerable<Quote>> FilterBy(IEnumerable<Quote> quotes, string target)
    {
